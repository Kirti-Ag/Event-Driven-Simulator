...
typedef struct FacilityHeap FacilityHeap;
enum FacilityName
{
    REGISTRATION,
    ECG,
...
FacilityHeap ECGHeap[1000];
int necg = 0;
FacilityHeap ECHOHeap[1000];
int necho = 0;
int n = 0;
Facility facility[6];
Package package[7];
...
void Initialization(char **argv)
{
    ...
    package[1].priority = 3;
    package[4].priority =1; package[5].priority = 1;
    package[6].priority = 2;
    facility[0].meanST = 5;
...
/*
*   AdjustFacilityHeap()- Adjust facility heap(for ECHO and ECG) based on Priority of the patient's package
*                         and timestamp for patients with same package
*/
void AdjustFacilityHeap(FacilityHeap *Heap, int *n)
{
    FacilityHeap Node = Heap[1];
    int itemTS = Heap[1].Node.timestamp;
    int itemP = Heap[1].priority;
    int j = 2;
    while (j <= *n)
    {
        if (j < *n && ((Heap[j].priority < Heap[j + 1].priority) || (Heap[j].priority == Heap[j + 1].priority && Heap[j].Node.timestamp > Heap[j + 1].Node.timestamp)))
            j++;
        if (Heap[j].priority <= itemP || (Heap[j].priority == itemP && Heap[j].Node.timestamp >= itemTS))
            break;
        Heap[j / 2] = Heap[j];
        j = j * 2;
    }
...
/*
*   InsertFacilityHeap()- Insert patients for facility ECG and ECHO into their respective heaps when
*                         facility's state is busy
*/
void InsertFacilityHeap(FacilityHeap *Heap, MinHeap node, int priority, int *n)
{
    *n = *n + 1;
    int i = *n;
    while (i > 1 && (Heap[i / 2].priority < priority ||
                     (Heap[i / 2].priority == priority && Heap[i / 2].Node.timestamp > node.timestamp)))
    {
        Heap[i] = Heap[i / 2];
        i = i / 2;
    }
...
/*
*   DelFacilityHeap()- Removes patient from the facility heap when another patient has finished using it.
*/
void DelFacilityHeap(FacilityHeap *Heap, int *n, int facnum)
{
    if ((*n) > 0)
    {
        MinHeap Node = Heap[1].Node;
        Heap[1] = Heap[*n];
        *n = *n - 1;
        AdjustFacilityHeap(Heap, n);
        Service(facnum, Node);
    }
}
/*
...
void Service(int facnum, MinHeap patient)
{
    ...
    {
        if (facnum == 1)
            InsertFacilityHeap(ECGHeap, patient, package[patient.packagetype].priority, &necg);
        else if (facnum == 2)
            InsertFacilityHeap(ECHOHeap, patient, package[patient.packagetype].priority, &necho);
        else
...
void EndOfPackage(int facnum, MinHeap patient)
{
    ...
    if (facnum == 1)
        DelFacilityHeap(ECGHeap, &necg, 1);
    else if (facnum == 2)
        DelFacilityHeap(ECHOHeap, &necho, 2);
    else
        deQueue(facnum);
...
int main(int argc, char **argv)
{
    ...
    for (int i = 2; i < 6; i++)
    {
        if (facility[min].utiltime > facility[i].utiltime)
            min = i;
        if (facility[max].utiltime < facility[i].utiltime)
            max = i;
    }
