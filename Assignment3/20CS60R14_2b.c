...
void CreateFirstArrival(int packagetype, int meanAT)
{
    ...
    temp.facilityUsed = (bool *)malloc((package[packagetype].nooffac + 1) * sizeof(bool));
    for (int i = 0; i <= package[packagetype].nooffac; i++)
    {
        temp.facilityUsed[i] = false;
    }
    InsertMinHeap(temp);
}
/*
...
void CreateNewArrival(int packagetype, int timestamp)
{
    ...
    Node.facilityUsed = (bool *)malloc((package[packagetype].nooffac + 1) * sizeof(bool));
    for (int i = 0; i <= package[packagetype].nooffac; i++)
    {
        Node.facilityUsed[i] = false;
    }
    ...
    for (int days = 1; days <= 10; days++)
    {
        ...
        while (n > 0)
        {
            ...
            case 'A':
                ...
                Service(REGISTRATION, patient);
                break;

            case 'S':
                if (patient.lastfacilityused == REGISTRATION)
                {
                    facility[REGISTRATION].state = FREE;
                    deQueue(REGISTRATION);
                }
                int nextfacility = -1;
                int minWT = -1;
                int finish = 1;
                if (patient.packagetype == 1 || patient.packagetype == 6)
                {
                    for (int i = 1; i <= package[patient.packagetype].nooffac; i++)
                    {
                        if (patient.facilityUsed[i] == false)
                        {
                            finish = 0;
                            if (minWT == -1)
                            {
                                minWT = facilityqueuedet[i].size * facility[i].meanST;
                                nextfacility = i;
                            }
                            else if (minWT > facilityqueuedet[i].size * facility[i].meanST)
                            {
                                minWT = facilityqueuedet[i].size * facility[i].meanST;
                                nextfacility = i;
                            }
                ...
                case 1:
                    if (patient.lastfacilityused == REGISTRATION)
                    {
                        Service(nextfacility, patient);
                        break;
                    }
                    if (finish == 0)
                    {
                        facility[patient.lastfacilityused].state = 0;
                        deQueue(patient.lastfacilityused);
                        patient.facilityUsed[nextfacility] = true;
                        Service(nextfacility, patient);
                        break;
                    }
                    EndOfPackage(patient.lastfacilityused, patient);
                    break;
